...
vApp a_ u = do
  ...
  let
    ...
    primOpApp n ra ar = forceVal u >>= \u -> case T0 of
     ...
     where
      ...
      evalPrimOp _ _ = (impossible "src/D4_Eval.hs" 81)
  ...
  case a of
    ...
    _ -> (impossible "src/D4_Eval.hs" 95)
    ...
    _ | Just (T2 ref i) <- metaDep v -> lookupMeta ref >>= \case
      ...
      Nothing -> lookupMeta (metaRef i) >>= \case
        ...
        Just{} -> do
            r <- case v of
              WApp a b -> vApp a b
              _ -> (impossible "src/D4_Eval.hs" 122)
            updateMeta ref r
            changed r
    _ -> pure v
...
deepForce v_ = do
  v <- forceVal v_
  m <- go (v :. Nil)
  pure $ fromMaybe (lazy (impossible "src/D4_Eval.hs" 249)) $ lookupIM v m
 where
...
{-      
      WLam c -> do
        ...
        ret (Just u) (b :. Nil)
-}
      _ -> (undefined "src/D4_Eval.hs" 266)
     where
...
--      WSup{}    | Just n <- mn, body :. _ <- ts -> vLam n body
      _ -> (undefined "src/D4_Eval.hs" 278)
...
tmToRaw = go where
  go = \case
    TVar n     -> var n
    TLet n a b -> RLet (RVar n) N39  <$> go a <*> go b
    TGen e     -> N27 <@> (eval' emptyIM e >>= quoteNF >>= go)
    a :@. b    -> RApp <$> go a <*> go b
  ...
  var n = pure $ case isInfix n of True -> N23 :@ RVar n ; _ -> RVar n
...
walkTm sh t = do
 ...
 where
  tLet _    n _ t | not sh = RLetTy (RVar n) N62 <$> t
  tLet True n _ t = RLetTy (RVar n) N39 <$> t
  tLet _    n a t = RLet   (RVar n) N39 <$> tmToRaw a <*> t
