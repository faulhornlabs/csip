...
updateClosed a b = do
  ...
  fe <- deepForce v
  case fe of
    WMeta r | a === r -> (impossible "src/E1_Unify.hs" 14)
    _ -> update a fe
...
computePrunes ps = f Nil emptyIM ps  where
  f acc m Nil = Just $ map (\d -> T2 d $ fromMaybe (lazy (impossible "src/E1_Unify.hs" 50)) $ lookupIM d m) acc
...
closeTm v_ = do
  v <- forceVal v_
  m <- go (v :. Nil)
  case computePrunes $ fromMaybe (lazy (impossible "src/E1_Unify.hs" 63)) $ lookupIM v m of
 ...
 where
  go = downUpIM down (\_ -> pure) (\_ -> pure) up where
    ...
    up v ln (map snd -> ts) = case v of
      ...
      WMeta{} -> pure mempty
      _ -> (impossible "src/E1_Unify.hs" 92)
