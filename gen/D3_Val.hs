...
instance PPrint Val where
  pprint = \case
    ...
    WString n     -> pprint n
    WVar n        -> N72    <@> pprint n
    WSup _ c vs   -> N79    <@> pprint c <@> pprint vs
    WCon n        -> N80    <@> pprint n
    WMeta_ n _ _  -> N81   <@> pprint n
    WApp a b      -> N77      <@> pprint a <@> pprint b
    WPrimOp n _ _ -> N82 <@> pprint n
    WFun n _      -> N83    <@> pprint n
    _ -> (undefined "src/D3_Val.hs" 121)
...
lookupDictFun = metaFun N90 $ MkRuleRef $ topRef mainReset N91

{-# noinline superClassesFun #-}
superClassesFun = metaFun N92 $ MkRuleRef $ topRef mainReset N91

{-# noinline succViewFun #-}
succViewFun = WFun N93 $ MkRuleRef $ topRef mainReset N91

{-# noinline consViewFun #-}
consViewFun = WFun N94 $ MkRuleRef $ topRef mainReset N91

{-# noinline wordToNatFun #-}
wordToNatFun = WFun N95 $ MkRuleRef $ topRef mainReset N91

{-# noinline matchRetFun #-}
matchRetFun = WFun N96 $ MkRuleRef $ topRef_ mainReset $ nameOf N75 >>= \n -> vLam_ n $ TVar n

{-# noinline tGenFun #-}
tGenFun = WFun N84 $ MkRuleRef $ topRef_ mainReset $ nameOf N75 >>= \n -> vLam_ n $ N97 :@. TVar n

mkFun_ :: SName -> Maybe Val
mkFun_ = \case
  N93     -> Just succViewFun
  N94     -> Just consViewFun
  N90   -> Just lookupDictFun
  N92 -> Just superClassesFun
  N95    -> Just wordToNatFun
  N96     -> Just matchRetFun
  N84         -> Just tGenFun
  _ -> Nothing

mkFun :: SName -> Mem Val
mkFun n = maybe (WFun n . MkRuleRef <$> newRef N91) pure $ mkFun_ n

mkBuiltin :: SName -> Val
mkBuiltin n = case n of
  N98  -> f 2 \d -> \case WString va :. WString vb :. Nil -> vString $ va <> vb; _ -> d
  N99      -> f 2 \d -> \case WString va :. WString vb :. Nil -> pure $ cBool (va == vb); _ -> d
  N100    -> f 2 \d -> \case WNat va :. WString vb :. Nil -> vString $ takeStr va vb; _ -> d
  N101    -> f 2 \d -> \case WNat va :. WString vb :. Nil -> vString $ dropStr va vb; _ -> d
  N102    -> f 1 \d -> \case WNat va :. Nil -> vNat $ max 0 $ va - 1; _ -> d
  N103    -> f 2 \d -> \case WNat va :. WNat vb :. Nil -> vNat $ va + vb; _ -> d
  N104    -> f 2 \d -> \case WNat va :. WNat vb :. Nil -> vNat $ va * vb; _ -> d
  N105    -> f 2 \d -> \case WNat va :. WNat vb :. Nil -> vNat $ va `div` vb; _ -> d
  N106    -> f 2 \d -> \case WNat va :. WNat vb :. Nil -> vNat $ va `mod` vb; _ -> d
  N107     -> f 2 \d -> \case WNat va :. WNat vb :. Nil -> pure $ cBool (va == vb); _ -> d
  n | Just v <- mkFun_ n -> v
  n -> WCon n
