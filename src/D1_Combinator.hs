...
instance PPrint c => PPrint (PTm c) where
  pprint = \case
    TVar n     -> "Var"B <@> pprint n
    TLet n a b -> "= ;"B <@> pprint n <@> pprint a <@> pprint b
...
create a b = nameOf "n"B <&> GSup a b
{-
...
mkProg n t_ = do
 ...
 where
  ...
  mapProg p res args = go (fromListIM $ numberWith (flip T2) 0 args) (T2 (length args) Nil) p
   where
    go m fs Nil = T2 Nil $ T2 (fst fs) $ fromMaybe (lazy $impossible) $ lookupIM res m 
    go m fs (op :. ops) = case op of
      Free g
        | Just i <- lookupIM g m -> go m (free i fs) ops
      Create' g k xs
        | T2 i fs <- newReg fs
        , is <- xs <&> \x -> fromMaybe (lazy $impossible) $ lookupIM x m
